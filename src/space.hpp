...
namespace nl {
	...
	public:
		...
		constexpr u32 volume() { return X * Z * Y; };
	...
	private:
		...
		constexpr T& ref(u32 ix, u32 iy, u32 iz) { return ref(iy * X * Z + ix * Z + iz); };
		constexpr T ref(u32 ix, u32 iy, u32 iz) const { return ref(iy * X * Z + ix * Z + iz); };
	...
	public:
		...
		constexpr space(const T& srcVal) {
			auto init = [&srcVal](T& val, u32 ix, u32 iy, u32 iz) { val = srcVal; };
			forEach(init);
		};
	};
};
...
template<typename T, nl::u32 X, nl::u32 Y, nl::u32 Z> std::ostream& operator<<(std::ostream& out, const nl::space<T, X, Y, Z>& rsp) {
	auto log = [&out](const T& val, nl::u32 ix, nl::u32 iy, nl::u32 iz) mutable { out << val << (ix == (X - 1) ? (iy == (Y - 1) ? (iz == (Z - 1) ? "" : "\n\n") : "\n") : " "); };
	rsp.forEach(log);
	return out << "\n";
};